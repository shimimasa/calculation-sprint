...
## Playwright 実行結果（証跡）
- Playwright (chromium) は起動時に SIGSEGV でクラッシュし、E2E 実行が不可能でした。
- 例外ログ抜粋:
...
> **補足**: Playwright がクラッシュする制約のため、全テストは **Manual 手順（代替）** を記載し、実施結果は **Fail（Blocked）** としています。Playwright が復旧した場合は再実行してください。
...
> 60秒待機せずに「タイムアップ直前」を再現するため、デバッグ API を使用する手順です。

1. ゲーム開始後、ブラウザの DevTools コンソールで以下を実行:
   ```js
   window.app?.debug?.setTimeLeft?.(2)
   ```
2. 画面上のタイマーが **2秒** 付近に縮むことを確認。
3. **1秒未満** になる直前に「確定」または Enter を実行。
4. タイムアップ遷移と競合してもクラッシュしないことを確認。

> もし `setTimeLeft` が未提供の場合は、既存テストのように `setTimeLeft(1)` を利用し、1秒待ってタイムアップ遷移の安定性を確認してください。
